...
#"C:/pierre/data"
base_dir =  "/mnt/data2/mosab/stelaris/mosab_ipsn_301120/sample2/stack2" #replace by the path to the folder
#containing the data

#C:/pierre/analysis
out_dir = "/tmp/a" #replace by the path to the folder
...
chan_names = {0: "Synaptophysin", 1: "Tubulin", 2: "Homer"}

default_sigmas = [0.1, 0.1, 0.1]
...
for cnt, fname in enumerate(shuffle_ret(exp_dirs)):
    ...
    if not force and path.isfile(chan_file):
        ...
        log("  " + chan_names[0], logf)
        ran[0] = True
        imgs_1 = gaussian(imgs[:,0,:,:], sigma=sigmas[0])
    ...
    log("    Found {} {} clusters"
        .format(len(labs_props[0]), chan_names[0]), logf)
    ...
    log("    Found {} {} clusters"
        .format(len(labs_props[2]), chan_names[2]), logf)
    ...
    if params == {} or any(ran):
        for k in range(3):
            if ran[k]:
                params["th_{}".format(chan_names[k])] = ths[k]
                params["sigma_{}".format(chan_names[k])] = sigmas[k]
    ...
    if force or not path.isfile(path.join(out_exp_dir, "clusts.pkl")):
        ...
        with open(path.join(out_exp_dir, "clusts.pkl"), "wb") as f:
            pickle.dump({"sigmas": default_sigmas,
